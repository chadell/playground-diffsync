...
class NautobotRemote(DiffSync):
    ...
    def load(self):
        ...
        region_data = requests.get(
            f"{self.url}/api/dcim/regions/", headers=self.headers, params={"limit": 0}
        ).json()
        regions = region_data["results"]
        while region_data["next"]:
            region_data = requests.get(
                region_data["next"], headers=self.headers, params={"limit": 0}
            ).json()
        ...
        for region_entry in regions:
            region = self.region(
                ...
                parent_name=region_entry["parent"]["name"]
                if region_entry["parent"]
                else None,
                pk=region_entry["id"],
            )
            self.add(region)
        ...
        site_data = requests.get(
            f"{self.url}/api/dcim/sites/", headers=self.headers, params={"limit": 0}
        ).json()
        sites = site_data["results"]
        while site_data["next"]:
            site_data = requests.get(
                site_data["next"], headers=self.headers, params={"limit": 0}
            ).json()
        ...
        for site_entry in sites:
            site = self.site(
                ...
                if site_entry["status"]
                else "active",
                region_name=site_entry["region"]["name"]
